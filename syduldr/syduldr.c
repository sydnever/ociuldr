/* 
* syduldr [Song Yuandong*UnLoader]
...
/* oratypes.h
There are many terrible typedefs.
...
{
    if (p_err)
        OCIServerDetach(p_srv, p_err, (ub4)OCI_DEFAULT);
    if (p_srv)
        OCIHandleFree((dvoid *)p_srv, (CONST ub4)OCI_HTYPE_SERVER);
    if (p_svc)
        OCIHandleFree((dvoid *)p_svc, (CONST ub4)OCI_HTYPE_SVCCTX);
    if (p_err)
        OCIHandleFree((dvoid *)p_err, (CONST ub4)OCI_HTYPE_ERROR);
    if (p_sess)
...
sword get_columns(OCIStmt *p_stmt, struct COLUMN *collist)
{
    ...
    for (col = 0; col < numcols; col++)
    {
        ...
        if (param->enclose_len)
            tempcol->colwidth = tempcol->colwidth + 2 * param->enclose_len;
    ...
    if (strlen(param->tabname))
    {
        ...
        if (fp_ctl)
        {
            ...
            else
            {
                ...
                for (i = 0; i < strlen(param->record); i++)
    ...
    */
    for (i = 0; i < argc; i++)
    {
        ...
        else if (STRNCASECMP("long=", argv[i], 5) == 0)
        {
            ...
            if (param->lsize < 100)
                param->lsize = 100;
            if (param->lsize > MAX_CLOB_SIZE)
                param->lsize = MAX_CLOB_SIZE;
        }
